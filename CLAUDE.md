# 3aiii (agent-runtime) — Claude Code Instructions

## Project Overview
3aiii is the autonomous agent runtime for embodied AI agents (runs on a Raspberry Pi 5). Cognitive loop: SENSE → FEEL → THINK → ACT → REFLECT. Uses Groq cloud API (llama-3.1-8b-instant) for decisions.

## Key Architecture
- `src/loop/Heartbeat.js` — core cognitive loop
- `src/loop/SleepCycle.js` — sleep/wake + memory consolidation
- `src/cognition/` — InternalState (mood/energy), Think, DeltaDetector, RepetitionGuard
...
## Conventions
- Pure Node.js, minimal deps (ws, ollama, dotenv)
- No TypeScript, no build step
- ES modules throughout
- Config via env vars (see src/config.js)
...
### Remaining
- **No process watchdog**: No external mechanism to restart if Node hangs. Add systemd service or a health check endpoint.
